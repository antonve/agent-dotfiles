...
This local Pi package is built by Nix/Home Manager and loaded from `~/.local/share/agentbox-pi-setup`. It includes the upstream-inspired dashboard, ask-user, copy-all, system `fd`/`rg`, Git/model state, automatic summaries, and Herdr/Treehouse orchestration. Firecrawl is intentionally excluded.
