...
- Ports `8766`–`8783` are reserved and approved for agents running localhost-only local development servers, HTML report servers, previews, and similar temporary services. Bind only to `127.0.0.1` or `localhost`, choose an available port within that range, and avoid conflicts with active services. Port `8784` is reserved for the persistent T3 Code service.
