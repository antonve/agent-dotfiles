...
function layer(scope, projectRoot, userHome) {
  if (scope === "project") {
    return {
      ...
      receipt: path.join(projectRoot, ".codex/pstack-agentbox-agent-receipt.json"),
  ...
  if (scope === "user") {
    ...
    return {
      ...
      receipt: path.join(codexRoot, "pstack-agentbox-agent-receipt.json"),
...
function validateReceipt(receipt, scope, target) {
  if (!receipt) return;
  if (receipt.schema_version !== 1 || receipt.owner !== "pstack-agentbox/setup-pstack" || receipt.scope !== scope) {
...
export async function installAgents({
  ...
  const receipt = {
    schema_version: 1,
    owner: "pstack-agentbox/setup-pstack",
    scope,
